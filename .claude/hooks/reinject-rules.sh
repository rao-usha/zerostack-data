...
0. SPEC FIRST: Before editing any source code in app/, run /spec <task_type> <feature_name> to create a spec with skeleton tests. The check-spec-exists hook will BLOCK edits without an active spec. For trivial single-line fixes, write BYPASS_TRIVIAL to docs/specs/.active_spec.
...
6. AFTER CORRECTIONS: Run /feedback to capture the correction and update rubrics for future tasks.
EOF
