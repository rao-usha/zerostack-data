...
class PortfolioResearchAgent:
    ...
    async def collect_portfolio(
        ...
        logger.info(f"Starting portfolio collection for {context.investor_name} (id={context.investor_id})")
    ...
    def _should_continue(
        ...
        if len(current_results) >= self.min_companies_target:
            ...
            if len(unique_sources) >= self.min_sources:
                return False, f"Target coverage achieved: {len(current_results)} companies from {len(unique_sources)} sources"
        ...
        if len(current_results) < self.min_companies_target:
            return True, f"Coverage below target ({len(current_results)} < {self.min_companies_target}), continuing"
        
        # Single source only, need validation
        unique_sources = set(r.get("source_type") for r in current_results)
        if len(unique_sources) == 1:
            return True, "Single source detected, seeking validation from additional source"
                ...
                if existing:
                    ...
                    self.db.execute(
                        ...
                        """),
                        {
                            ...
                            "investment_amount_usd": company_data.get("investment_amount_usd"),
                    ...
                    self.db.execute(
                        ...
                        """),
                        {
                            ...
                            "shares_held": company_data.get("shares_held"),
                            "market_value_usd": company_data.get("market_value_usd"),
                            "ownership_percentage": company_data.get("ownership_percentage"),
                            ...
                            "confidence_level": company_data.get("confidence_level", "medium"),
    ...
    def _create_result(
        ...
        successful_strategies = [
            r.strategy_name for r in strategy_results if r.success
        ]
        ...
        return {
            ...
            "updated_companies": store_result["updated_companies"] if store_result else 0,
