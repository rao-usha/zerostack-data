...
class AnnualReportStrategy(BaseStrategy):
    ...
    REPORT_KEYWORDS = [
        "annual report", "cafr", "comprehensive annual financial report",
        "investment report", "financial report", "fiscal year report",
        "annual investment report", "pension fund report"
    ]
    
    # Keywords to find portfolio sections in PDFs
    PORTFOLIO_SECTION_KEYWORDS = [
        "schedule of investments", "investment holdings", "portfolio holdings",
        "securities held", "equity holdings", "fixed income holdings",
        "alternative investments", "private equity investments",
        "investment schedule", "asset allocation"
    ]
    ...
    def is_applicable(self, context: InvestorContext) -> Tuple[bool, str]:
        ...
        if context.lp_type == "public_pension":
            return True, "Public pensions are required to publish annual reports (CAFRs)"
        ...
        if context.website_url:
            ...
            public_keywords = ["pension", "retirement", "state", "city", "county", "university"]
    ...
    async def _find_report_links(self, website_url: str) -> List[str]:
        ...
        try:
            ...
            ir_paths = ["/investors", "/investor-relations", "/financials", "/reports", "/about/financials"]
            for path in ir_paths:
                ...
                if ir_response and ir_response.status_code == 200:
                    ...
                    for a_tag in ir_soup.find_all('a', href=True):
                        href = a_tag['href'].lower()
                        if href.endswith('.pdf') or any(kw in href for kw in ['annual', 'report', 'cafr']):
    ...
    def _extract_holdings_from_pdf(self, pdf_content: bytes, source_url: str) -> List[Dict[str, Any]]:
            ...
            with pdfplumber.open(io.BytesIO(pdf_content)) as pdf:
                ...
                for i, page in enumerate(pdf.pages[:pages_to_scan]):
                    ...
                    if in_portfolio_section:
                        ...
                        for table in tables:
                            table_holdings = self._parse_holdings_table(table, source_url)
                ...
                if not holdings and portfolio_text:
                    text_holdings = self._extract_holdings_from_text(portfolio_text, source_url)
    ...
    def _parse_holdings_table(self, table: List[List], source_url: str) -> List[Dict[str, Any]]:
        ...
        for i, h in enumerate(header_lower):
            if any(kw in h for kw in ['security', 'issuer', 'company', 'name', 'description']):
                name_col = i
    ...
    def _extract_holdings_from_text(self, text: str, source_url: str) -> List[Dict[str, Any]]:
        ...
        for line in lines:
            ...
            if re.match(r'^[A-Z][A-Za-z\s\.\,\&\-]+$', line):
                ...
                skip_patterns = ['total', 'schedule', 'investment', 'holdings', 'portfolio']
