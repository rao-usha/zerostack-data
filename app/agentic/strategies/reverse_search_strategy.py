...
class ReverseSearchStrategy(BaseStrategy):
    ...
    def is_applicable(self, context: InvestorContext) -> Tuple[bool, str]:
        ...
        return (
            True,
            "Reverse search is always worth trying (portfolio companies often list investors)",
        )
    ...
    async def execute(self, context: InvestorContext) -> StrategyResult:
        ...
        try:
            ...
            reasoning_parts.append(
                f"Searching for companies that mention '{context.investor_name}'"
            )
            ...
            reasoning_parts.append(
                f"Found {len(search_results)} potential company websites"
            )
            ...
            for result in search_results[: self.MAX_SEARCH_RESULTS]:
                ...
                company_info = await self._verify_and_extract(
                    url, context.investor_name
                )
                requests_made += 1
                ...
                if company_info:
                    ...
                    reasoning_parts.append(
                        f"Verified: {company_info.get('company_name')} ({domain})"
                    )
            ...
            reasoning_parts.append(
                f"Verified {len(companies)} companies mentioning {context.investor_name}"
            )
    ...
    async def _verify_and_extract(
        self, url: str, investor_name: str
