...
class SEC13FStrategy(BaseStrategy):
    ...
    def is_applicable(self, context: InvestorContext) -> Tuple[bool, str]:
        ...
        if context.aum_usd and context.aum_usd >= 100_000_000:
            return (
                True,
                f"AUM ${context.aum_usd/1e9:.1f}B >= $100M threshold for 13F filing",
            )
        ...
        try:
            ...
            for holding in holdings:
                company = {
                    ...
                    "shares_held": str(
                        holding.get("shrsOrPrnAmt", {}).get("sshPrnamt", 0)
                    ),
                    "market_value_usd": str(
                        holding.get("value", 0) * 1000
                    ),  # 13F reports in thousands
            ...
            reasoning_parts.append(
                f"Found {len(companies)} holdings from 13F filing dated {most_recent.get('filingDate')}"
            )
        ...
        try:
            ...
            if response2.status_code == 200:
                ...
                if cik_match2:
                    ...
                    return (
                        cik,
                        f"Found CIK {cik} via company search for '{investor_name}'",
                    )
    ...
    async def _get_13f_filings(self, cik: str) -> Tuple[List[Dict[str, Any]], str]:
        ...
        try:
            ...
            if filings:
                return (
                    filings[:5],
                    f"Found {len(filings)} 13F filings, most recent: {filings[0].get('filingDate')}",
                )
            else:
    ...
    async def _parse_13f_filing(
        ...
        try:
            ...
            if xml_response.status_code != 200:
                return (
                    [],
                    f"Failed to download info table: status {xml_response.status_code}",
                )
