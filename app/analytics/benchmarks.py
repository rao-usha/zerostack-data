...
class BenchmarkService:
    ...
    def calculate_sector_benchmark(
        self, investor_type: str, subtype: Optional[str] = None
    ) -> Dict:
        """
        ...
        if subtype:
            investors = [
                i
                for i in investors
                if (i.get("subtype") or "").lower() == subtype.lower()
            ]
        ...
        for inv in investors:
            ...
            for sector_data in allocs.get("sectors", []):
                sector_allocations[sector_data["sector"]].append(
    ...
    def compare_to_benchmark(self, investor_id: int, investor_type: str) -> Dict:
        ...
        investor_sectors = {
            s["sector"]: s["allocation"] for s in investor_allocs.get("sectors", [])
        }
