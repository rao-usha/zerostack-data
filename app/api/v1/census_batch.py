...
class BatchStateRequest(BaseModel):
    ...
    years: List[int] = Field(
        ..., description="List of years to ingest", min_length=1, max_length=10
    )
    table_id: str = Field(..., description="Census table ID (e.g., B01001)")
    include_geojson: bool = Field(
        default=False, description="Include GeoJSON boundaries"
    )
...
async def batch_job_status(job_ids: str, db: Session = Depends(get_db)) -> dict:
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail="Invalid job_ids format. Use comma-separated integers.",
        )
