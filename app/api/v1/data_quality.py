...
from app.core import rule_seeder_service
...
def list_rule_types():
    ...
    return {
        "rule_types": [
            ...
            {
                "type": "custom_sql",
                "description": "Custom SQL query that returns violation count (0 = pass)",
                "parameters": {
                    "condition": "SQL with {table} placeholder",
                    "timeout_seconds": "number (default 30, max 60)",
                },
                "requires_column": False,
                "example": {
                    "condition": 'SELECT COUNT(*) FROM {table} WHERE "amount" < 0'
                },
            },
            {
                "type": "comparison",
                "description": "Compare two columns using an operator",
                "parameters": {
                    "operator": "one of <, >, <=, >=, =, !=",
                    "compare_column": "column to compare against",
                },
                "requires_column": True,
                "example": {"operator": "<=", "compare_column": "end_date"},
            },
        ],
