...
class SimilarInvestorResponse(BaseModel):
    ...
    investor_type: str = Field(..., description="Type: public_pension, sovereign_wealth, etc.")
    ...
    sample_overlap: List[str] = Field(default_factory=list, description="Sample of shared companies")
...
class CompanyRecommendationResponse(BaseModel):
    ...
    held_by_similar_count: int = Field(..., description="How many similar investors hold this")
    held_by_investors: List[str] = Field(default_factory=list, description="Names of investors who hold this")
    confidence_score: float = Field(..., description="Confidence (0-1) based on similar investor coverage")
...
class RecommendationsResponse(BaseModel):
    ...
    based_on_similar_count: int = Field(..., description="Number of similar investors used for recommendations")
...
class OverlapResponse(BaseModel):
    ...
    overlap_count: int = Field(..., description="Number of shared holdings")
    overlap_percentage_a: float = Field(..., description="% of investor A's portfolio that overlaps")
    overlap_percentage_b: float = Field(..., description="% of investor B's portfolio that overlaps")
    ...
    shared_companies: List[str] = Field(default_factory=list, description="List of shared companies")
...
async def get_similar_investors(
    investor_id: int,
    investor_type: Optional[str] = Query(None, description="Filter by type: public_pension, sovereign_wealth, endowment"),
    ...
    **Examples:**
    ...
    if investor_type:
        valid_types = {"public_pension", "sovereign_wealth", "endowment", "family_office"}
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to find similar investors: {str(e)}")


@router.get("/recommended/{investor_id}", response_model=RecommendationsResponse)
async def get_recommended_companies(
    investor_id: int,
    similar_count: int = Query(10, ge=1, le=50, description="Number of similar investors to consider"),
...
async def get_portfolio_overlap(
    ...
    **Examples:**
    ...
    if investor_a == investor_b:
        raise HTTPException(status_code=400, detail="Cannot compare an investor to itself")
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to calculate overlap: {str(e)}")
