...
class MergeRequest(BaseModel):
    ...
    source_entity_id: int = Field(..., description="Entity to merge FROM (will be deleted)")
    target_entity_id: int = Field(..., description="Entity to merge INTO (will be kept)")
...
class SplitRequest(BaseModel):
    ...
    aliases_to_split: List[str] = Field(..., description="Alias names to move to new entity")
...
async def resolve_entity(
    ...
    return ResolveResponse(
        ...
        alternatives=[
            AlternativeMatch(**alt) for alt in result.alternatives
        ]
    )


@router.post("/resolve", response_model=ResolveResponse)
async def resolve_entity_post(
...
async def search_entities(
    ...
    entities = resolver.search_entities(
        query=q,
        entity_type=entity_type,
        limit=limit
    )
    ...
    return EntitySearchResponse(
        entities=[entity_to_response(e) for e in entities],
        total=len(entities)
    )
...
@router.get("/by-identifier/{identifier_type}/{identifier_value}",
