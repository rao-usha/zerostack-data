...
class ExtractFromWebsitesRequest(BaseModel):
    ...
    office_ids: Optional[List[int]] = Field(None, description="Optional list of family office IDs (None = all with websites)")
    max_concurrency: int = Field(1, description="Max concurrent requests (default 1 for privacy)")
    delay_seconds: float = Field(5.0, description="Delay between requests (default 5 seconds)")
...
@router.post("/family-offices/contacts/extract-from-websites", response_model=ExtractFromWebsitesResponse)
async def extract_contacts_from_websites(
    request: ExtractFromWebsitesRequest,
    ...
    **Privacy Controls:**
    ...
    """
    try:
        logger.info(f"Starting website contact extraction for office_ids: {request.office_ids}")
...
def get_contact_summary(db: Session = Depends(get_db)):
    ...
    try:
        ...
        fos_with_contacts = db.execute(text("""
            SELECT COUNT(DISTINCT family_office_id) FROM family_office_contacts
        """)).scalar()
        
        # Total contacts
        total_contacts = db.execute(text("SELECT COUNT(*) FROM family_office_contacts")).scalar()
        
        # By role
        by_role_result = db.execute(text("""
        ...
        # By source
        by_source_result = db.execute(text("""
            SELECT 
                CASE 
        ...
        # By confidence
        by_confidence_result = db.execute(text("""
...
@router.get("/family-offices/{office_id}/contacts", response_model=List[ContactResponse])
def get_contacts_for_office(
    office_id: int,
    db: Session = Depends(get_db)
):
    """
...
@router.post("/family-offices/contacts/import-manual", response_model=ManualImportResponse)
        ...
        for row_num, row in enumerate(csv_reader, start=2):  # Start at 2 (1 is header)
            try:
                ...
                if not row.get('office_id') or not row.get('full_name'):
                    validation_errors.append(f"Row {row_num}: Missing required fields (office_id, full_name)")
                    rows_skipped += 1
                    continue
                ...
                office_exists = db.execute(text("""
                    SELECT COUNT(*) FROM family_offices WHERE id = :office_id
                """), {'office_id': office_id}).scalar()
                
                if office_exists == 0:
                    validation_errors.append(f"Row {row_num}: Family office ID {office_id} not found")
                    rows_skipped += 1
                    continue
                ...
                if existing > 0:
                    validation_errors.append(f"Row {row_num}: Duplicate contact {row['full_name']}")
                    rows_skipped += 1
                    continue
...
def export_contacts(db: Session = Depends(get_db)):
    ...
    **Privacy Notice:** This data should only be used for legitimate business purposes.
    ...
    try:
        ...
        # Header
        writer.writerow([
            'Family Office', 'Name', 'Title', 'Role', 'Email', 'Phone',
            'Confidence', 'Source', 'Collected Date', 'Status'
        ])
