...
class DisasterDeclarationsIngestRequest(BaseModel):
    ...
    max_records: int = Field(
        default=50000, description="Maximum records to fetch", ge=1, le=100000
    )
...
class PAProjectsIngestRequest(BaseModel):
    ...
    state: Optional[str] = Field(
        None, description="Filter by state code", examples=["FL"], max_length=2
    )
    disaster_number: Optional[int] = Field(
        None, description="Filter by specific disaster number", examples=[4673]
    )
    max_records: int = Field(
        default=50000, description="Maximum records to fetch", ge=1, le=100000
    )
...
class HMAProjectsIngestRequest(BaseModel):
    ...
    state: Optional[str] = Field(
        None, description="Filter by state code", examples=["CA"], max_length=2
    )
    program_area: Optional[HMAProgram] = Field(
        None, description="Filter by program: HMGP, PDM, FMA, RFC"
    )
    max_records: int = Field(
        default=50000, description="Maximum records to fetch", ge=1, le=100000
    )
    ...
    **No API key required** - free public OpenFEMA API
    ...
    return create_and_dispatch_job(
        ...
        config={
            ...
            "state": request.state,
            "program_area": request.program_area.value
            if request.program_area
            else None,
            "max_records": request.max_records,
        },
