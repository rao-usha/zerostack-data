...
def list_boundary_datasets(db: Session = Depends(get_db)) -> List[dict]:
    ...
    results = (
        ...
        .group_by(GeoJSONBoundaries.dataset_id, GeoJSONBoundaries.geo_level)
        .order_by(GeoJSONBoundaries.dataset_id)
        .all()
    )
...
def list_boundaries(
    ...
    """
    boundaries = (
        db.query(GeoJSONBoundaries)
        .filter(GeoJSONBoundaries.dataset_id == dataset_id)
        .limit(limit)
        .all()
    )
    ...
    if not boundaries:
        raise HTTPException(
            status_code=404, detail=f"No boundaries found for dataset: {dataset_id}"
        )
...
def get_boundary(
    dataset_id: str, geo_id: str, db: Session = Depends(get_db)
) -> BoundaryWithGeoJSON:
    """
...
def get_feature_collection(
    ...
    geo_ids: Optional[str] = Query(
        None, description="Comma-separated list of geo_ids to include"
    ),
    db: Session = Depends(get_db),
) -> dict:
    """
    ...
    return {
        ...
        "features": features,
        "metadata": {"dataset_id": dataset_id, "feature_count": len(features)},
    }
