...
def get_dataset_variables(
    ...
    variables = (
        ...
        .order_by(CensusVariableMetadata.variable_name)
        .all()
    )

    if not variables:
        raise HTTPException(
            status_code=404, detail=f"No metadata found for dataset: {dataset_id}"
        )
...
def search_variables(
    ...
    query: str = Query(
        ..., description="Search term (searches in label and concept)", min_length=2
    ),
...
def get_column_info(
    dataset_id: str, column_name: str, db: Session = Depends(get_db)
) -> VariableInfo:
    """
