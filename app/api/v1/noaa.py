...
class NOAAIngestionRequest(BaseModel):
    ...
    location_id: Optional[str] = Field(
        None, description="Location filter (e.g., 'FIPS:06' for California)"
    )
    station_id: Optional[str] = Field(
        None, description="Station filter (e.g., 'GHCND:USW00023174')"
    )
    data_type_ids: Optional[List[str]] = Field(
        None, description="Data types to fetch (e.g., ['TMAX', 'TMIN'])"
    )
    ...
    requests_per_second: float = Field(
        4.0, description="Rate limit (requests per second)"
    )
...
async def ingest_noaa(request: NOAAIngestionRequest) -> NOAAIngestionResponse:
    """
...
async def list_datasets() -> NOAADatasetsResponse:
    ...
    return NOAADatasetsResponse(datasets=dataset_infos, count=len(dataset_infos))
...
async def get_locations(
    ...
    location_category_id: Optional[str] = Query(
        None, description="Location category (e.g., ST for states)"
    ),
...
async def get_stations(
    ...
    location_id: Optional[str] = Query(
        None, description="Location filter (e.g., FIPS:06)"
    ),
    limit: int = Query(100, description="Maximum results", le=1000),
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        try:
            stations = await client.get_stations(
                dataset_id=dataset_id, location_id=location_id, limit=limit
            )
...
async def get_data_types(
    ...
    **Example:**
    ...
    try:
        ...
        try:
            data_types = await client.get_data_types(dataset_id=dataset_id, limit=limit)
