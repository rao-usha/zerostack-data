    ...
    return [
        {
            ...
            "earthquake_score": float(c.earthquake_score)
            if c.earthquake_score
            else None,
...
async def get_county_nri(
    ...
    county = (
        db.query(NationalRiskIndex)
        .filter(NationalRiskIndex.county_fips == county_fips)
        .first()
    )
    ...
    return {
        ...
        "individual_scores": {
            "earthquake": float(county.earthquake_score)
            if county.earthquake_score
            else None,
            ...
            "hurricane": float(county.hurricane_score)
            if county.hurricane_score
            else None,
            "wildfire": float(county.wildfire_score) if county.wildfire_score else None,
        },
        "resilience": {
            "social_vulnerability": float(county.social_vulnerability)
            if county.social_vulnerability
            else None,
            "community_resilience": float(county.community_resilience)
            if county.community_resilience
            else None,
            "expected_annual_loss": float(county.expected_annual_loss)
            if county.expected_annual_loss
            else None,
        },
    }
...
async def get_state_nri_summary(
    ...
    counties = (
        db.query(NationalRiskIndex)
        .filter(NationalRiskIndex.state == state.upper())
        .order_by(NationalRiskIndex.risk_score.desc().nullslast())
        .all()
    )
...
async def get_seismic_risk_at_location(
    ...
    result = (
        db.query(SeismicHazard, distance_expr)
        .filter(
        ...
        "faults": [
            {
                ...
                "slip_rate_mm_yr": float(f.slip_rate_mm_yr)
...
async def get_climate_at_location(
    ...
    distance_expr = (
        ...
        * func.acos(
            ...
            * func.cos(func.radians(ClimateData.longitude) - func.radians(lng))
            + func.sin(func.radians(lat)) * func.sin(func.radians(ClimateData.latitude))
        )
    ).label("distance_miles")

    result = (
        db.query(ClimateData, distance_expr)
        .filter(
    ...
    if not result:
        return {"location": {"latitude": lat, "longitude": lng}, "climate_data": None}
    ...
    return {
        ...
        "temperature": {
            "avg_annual_f": float(climate.avg_temp_annual)
            if climate.avg_temp_annual
            else None,
            "avg_january_f": float(climate.avg_temp_jan)
            if climate.avg_temp_jan
            else None,
        ...
        "precipitation": {
            "annual_inches": float(climate.precip_annual_inches)
            if climate.precip_annual_inches
            else None,
            "snowfall_inches": float(climate.snowfall_annual_inches)
            if climate.snowfall_annual_inches
            else None,
        },
        "degree_days": {
...
async def find_nearby_environmental_facilities(
    ...
    distance_expr = (
        ...
        * func.acos(
            ...
            * func.cos(
                func.radians(EnvironmentalFacility.longitude) - func.radians(lng)
            )
            + func.sin(func.radians(lat))
    ...
    return {
        ...
        "summary": {
            ...
            "with_violations": sum(
                1 for f in facilities if f["violations_5yr"] and f["violations_5yr"] > 0
            ),
        },
    }
...
async def get_risk_score(
    ...
    env_count = (
        db.query(func.count(EnvironmentalFacility.id))
        .filter(
    ...
    return {
        ...
        "risk_level": (
            "High" if total_risk >= 60 else "Moderate" if total_risk >= 35 else "Low"
        ),
        "factors": {
            "seismic": {"pga": pga, "score": round(seismic_score, 1)},
            "climate": {
                "tornado": tornado_risk,
                "hurricane": hurricane_risk,
                "score": round(climate_score, 1),
            },
            "environmental": {
                "superfund_nearby": env_count,
...
async def get_risk_summary(db: Session = Depends(get_db)):
    ...
    return {
        ...
        "record_counts": {
            ...
            "environmental_facilities": db.query(
                func.count(EnvironmentalFacility.id)
            ).scalar(),
