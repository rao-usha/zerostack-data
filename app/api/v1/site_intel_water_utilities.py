...
from app.core.models_site_intel import (
    WaterMonitoringSite, PublicWaterSystem, WaterSystemViolation,
    NaturalGasPipeline, NaturalGasStorage, UtilityRate,
)

logger = logging.getLogger(__name__)

router = APIRouter(prefix="/site-intel/water-utilities", tags=["Site Intel - Water & Utilities"])
...
async def search_monitoring_sites(
    ...
    site_type: Optional[str] = Query(None, description="Filter by site type: stream, well, spring, lake"),
    has_streamflow: Optional[bool] = Query(None, description="Filter to sites with streamflow data"),
    has_groundwater: Optional[bool] = Query(None, description="Filter to sites with groundwater data"),
    ...
    return [
        {
            ...
            "drainage_area_sq_mi": float(s.drainage_area_sq_mi) if s.drainage_area_sq_mi else None,
            "aquifer_name": s.aquifer_name,
            "well_depth_ft": float(s.well_depth_ft) if s.well_depth_ft else None,
            "latest_streamflow_cfs": float(s.latest_streamflow_cfs) if s.latest_streamflow_cfs else None,
            "latest_gage_height_ft": float(s.latest_gage_height_ft) if s.latest_gage_height_ft else None,
            "measurement_date": s.measurement_date.isoformat() if s.measurement_date else None,
...
async def find_monitoring_sites_near(
    latitude: float = Query(..., ge=-90, le=90, description="Latitude of target location"),
    longitude: float = Query(..., ge=-180, le=180, description="Longitude of target location"),
    radius_miles: float = Query(50, ge=1, le=200, description="Search radius in miles"),
    site_type: Optional[str] = Query(None, description="Filter by site type: stream, well"),
    ...
    query = db.query(WaterMonitoringSite).filter(
        and_(
            WaterMonitoringSite.latitude.between(latitude - lat_range, latitude + lat_range),
            WaterMonitoringSite.longitude.between(longitude - lon_range, longitude + lon_range),
        )
    )
    ...
    for s in sites:
        if s.latitude and s.longitude:
            distance = haversine_distance(latitude, longitude, float(s.latitude), float(s.longitude))
...
async def get_monitoring_site(
    ...
    """
    site = db.query(WaterMonitoringSite).filter(
        WaterMonitoringSite.site_number == site_number
    ).first()

    if not site:
        raise HTTPException(status_code=404, detail=f"Monitoring site {site_number} not found")
    ...
    return {
        ...
        "drainage_area_sq_mi": float(site.drainage_area_sq_mi) if site.drainage_area_sq_mi else None,
        ...
        "latest_readings": {
            "streamflow_cfs": float(site.latest_streamflow_cfs) if site.latest_streamflow_cfs else None,
            "gage_height_ft": float(site.latest_gage_height_ft) if site.latest_gage_height_ft else None,
            "water_temp_c": float(site.latest_water_temp_c) if site.latest_water_temp_c else None,
            "dissolved_oxygen_mgl": float(site.latest_dissolved_oxygen) if site.latest_dissolved_oxygen else None,
            "ph": float(site.latest_ph) if site.latest_ph else None,
            "turbidity_ntu": float(site.latest_turbidity) if site.latest_turbidity else None,
            "measurement_date": site.measurement_date.isoformat() if site.measurement_date else None,
        },
        "data_availability": {
...
async def search_water_systems(
    ...
    pws_type: Optional[str] = Query(None, description="Type: community, transient, non_transient"),
    min_population: Optional[int] = Query(None, description="Minimum population served"),
    primary_source: Optional[str] = Query(None, description="Source: groundwater, surface_water"),
    ...
    if primary_source:
        query = query.filter(PublicWaterSystem.primary_source_name.ilike(f"%{primary_source}%"))
...
async def get_water_system(
    ...
    """
    system = db.query(PublicWaterSystem).filter(
        PublicWaterSystem.pwsid == pwsid.upper()
    ).first()
    ...
    return {
        ...
        "infrastructure": {
            ...
            "storage_capacity_mg": float(system.storage_capacity_mg) if system.storage_capacity_mg else None,
            "distribution_miles": float(system.distribution_miles) if system.distribution_miles else None,
        },
        "status": {
            "is_active": system.is_active,
            "compliance_status": system.compliance_status,
            "last_compliance_date": system.last_compliance_date.isoformat() if system.last_compliance_date else None,
        },
        "contact": {
        ...
        "collected_at": system.collected_at.isoformat() if system.collected_at else None,
    }
...
async def get_water_system_violations(
    ...
    is_health_based: Optional[bool] = Query(None, description="Filter to health-based violations"),
    ...
    if violation_type:
        query = query.filter(WaterSystemViolation.violation_type.ilike(f"%{violation_type}%"))
        ...
        "violations": [
            {
                ...
                "returned_to_compliance_date": v.returned_to_compliance_date.isoformat() if v.returned_to_compliance_date else None,
            }
            for v in violations
        ]
    }
...
async def search_gas_pipelines(
    state: Optional[str] = Query(None, description="Filter by origin or destination state"),
    pipeline_type: Optional[str] = Query(None, description="Type: interstate, intrastate"),
    operator: Optional[str] = Query(None, description="Filter by operator name"),
    min_capacity_mmcfd: Optional[float] = Query(None, description="Minimum capacity in MMcf/d"),
    limit: int = Query(50, ge=1, le=200),
    db: Session = Depends(get_db),
):
    """
...
async def search_gas_storage(
    state: Optional[str] = Query(None, description="Filter by state code"),
    storage_type: Optional[str] = Query(None, description="Type: depleted_field, salt_cavern, aquifer"),
    min_capacity_bcf: Optional[float] = Query(None, description="Minimum total capacity in Bcf"),
    limit: int = Query(50, ge=1, le=200),
    db: Session = Depends(get_db),
):
    """
    ...
    return [
        {
            ...
            "total_capacity_bcf": float(f.total_capacity_bcf) if f.total_capacity_bcf else None,
            "working_gas_bcf": float(f.working_gas_bcf) if f.working_gas_bcf else None,
            "deliverability_mmcfd": float(f.deliverability_mmcfd) if f.deliverability_mmcfd else None,
...
async def find_gas_storage_near(
    latitude: float = Query(..., ge=-90, le=90, description="Latitude of target location"),
    longitude: float = Query(..., ge=-180, le=180, description="Longitude of target location"),
    radius_miles: float = Query(100, ge=1, le=300, description="Search radius in miles"),
    ...
    query = db.query(NaturalGasStorage).filter(
        and_(
            NaturalGasStorage.latitude.between(latitude - lat_range, latitude + lat_range),
            NaturalGasStorage.longitude.between(longitude - lon_range, longitude + lon_range),
        )
    )
    ...
    for f in facilities:
        if f.latitude and f.longitude:
            distance = haversine_distance(latitude, longitude, float(f.latitude), float(f.longitude))
            if distance <= radius_miles:
                results.append({
                    "id": f.id,
                    "facility_id": f.facility_id,
...
async def search_utility_rates(
    state: Optional[str] = Query(None, description="Filter by state code"),
    customer_class: Optional[str] = Query(None, description="Class: residential, commercial, industrial"),
    ...
    has_demand_charges: Optional[bool] = Query(None, description="Filter to rates with demand charges"),
    limit: int = Query(100, ge=1, le=500),
    db: Session = Depends(get_db),
):
    """
    ...
    return [
        {
            ...
            "demand_charge_kw": float(r.demand_charge_kw) if r.demand_charge_kw else None,
            "fixed_monthly_charge": float(r.fixed_monthly_charge) if r.fixed_monthly_charge else None,
...
async def compare_utility_rates(
    states: str = Query(..., description="Comma-separated state codes (e.g., 'TX,CA,OH')"),
    ...
    for state in state_list:
        rates = db.query(UtilityRate).filter(
            and_(
        ...
        if rates:
            energy_rates = [float(r.energy_rate_kwh) for r in rates if r.energy_rate_kwh]
            demand_charges = [float(r.demand_charge_kw) for r in rates if r.demand_charge_kw]

            results[state] = {
                "state": state,
                "customer_class": customer_class.lower(),
                "rate_count": len(rates),
                "avg_energy_rate_kwh": round(sum(energy_rates) / len(energy_rates), 6) if energy_rates else None,
                "min_energy_rate_kwh": round(min(energy_rates), 6) if energy_rates else None,
                "max_energy_rate_kwh": round(max(energy_rates), 6) if energy_rates else None,
                "avg_demand_charge_kw": round(sum(demand_charges) / len(demand_charges), 2) if demand_charges else None,
    ...
    return {
        ...
        "lowest_cost_state": sorted_results[0]["state"] if sorted_results and sorted_results[0].get("avg_energy_rate_kwh") else None,
    }
...
async def get_utility_rates(
    ...
    rates = query.order_by(UtilityRate.customer_class, UtilityRate.rate_schedule_name).all()

    if not rates:
        raise HTTPException(status_code=404, detail=f"No rates found for utility {utility_id}")
        ...
        "rate_schedules": [
            {
                ...
                "pricing": {
                    ...
                    "demand_charge_kw": float(r.demand_charge_kw) if r.demand_charge_kw else None,
                    "fixed_monthly_charge": float(r.fixed_monthly_charge) if r.fixed_monthly_charge else None,
                    "minimum_charge": float(r.minimum_charge) if r.minimum_charge else None,
                },
                "features": {
                    "has_time_of_use": r.has_time_of_use,
                    "has_demand_charges": r.has_demand_charges,
