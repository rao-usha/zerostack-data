...
class InviteMemberRequest(BaseModel):
    ...
    role: str = Field("member", pattern="^(viewer|member|admin)$", description="Role to assign")
...
def update_workspace(
    ...
    try:
        ...
        workspace = service.update_workspace(workspace_id, current_user["user_id"], updates)
        return workspace
...
def list_invitations(workspace_id: int, current_user: dict = Depends(get_current_user)):
    ...
    try:
        service = WorkspaceService(db)
        invitations = service.get_pending_invitations(workspace_id, current_user["user_id"])
        return {
            "workspace_id": workspace_id,
...
def update_member_role(
    ...
    try:
        ...
        if not updated:
            raise HTTPException(status_code=404, detail="Member not found")
        return {"message": "Role updated successfully", "user_id": user_id, "role": request.role}
    except PermissionError as e:
        raise HTTPException(status_code=403, detail=str(e))
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
