...
class Settings(BaseSettings):
    ...
    bea_api_key: Optional[str] = Field(
        default=None,
        description="BEA API key (UserID) - required for BEA operations"
    )
    ...
    safegraph_api_key: Optional[str] = Field(
        default=None,
        description="SafeGraph API key for foot traffic patterns data"
    )
    
    # Placer.ai API (Optional - enterprise pricing)
    placer_api_key: Optional[str] = Field(
        default=None,
        description="Placer.ai API key for retail analytics"
    )
    ...
    agentic_max_concurrent: int = Field(
        ...
        ge=1,
        le=10,
        description="Maximum concurrent agentic requests"
    )
