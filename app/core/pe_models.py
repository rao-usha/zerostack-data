...
# =============================================================================
# PORTFOLIO MONITORING TABLES
# =============================================================================


class PEPortfolioSnapshot(Base):
    """
    Point-in-time snapshot of a portfolio company's key metrics.

    Used by the portfolio monitor to detect changes over time.
    """

    __tablename__ = "pe_portfolio_snapshots"
    ...
    # Exit readiness
    exit_score = Column(Numeric(6, 2))
    exit_grade = Column(String(2))
    ...
    # Leadership
    leadership_count = Column(Integer)
    has_ceo = Column(Boolean, default=False)
    has_cfo = Column(Boolean, default=False)
    ...
    __table_args__ = (
        Index("ix_portfolio_snapshot_company_date", "company_id", "snapshot_date"),
    )


class PEAlertSubscription(Base):
    """
    Subscription to PE alert types for a firm.
    """

    __tablename__ = "pe_alert_subscriptions"
    ...
    __table_args__ = (
        UniqueConstraint("firm_id", "alert_type", name="uq_pe_alert_sub_firm_type"),
    )


class PEAlert(Base):
    """
    Alert record generated by portfolio monitoring.
    """

    __tablename__ = "pe_alerts"

    id = Column(Integer, primary_key=True, autoincrement=True)
    firm_id = Column(Integer, ForeignKey("pe_firms.id"), nullable=False, index=True)
    company_id = Column(Integer, ForeignKey("pe_portfolio_companies.id"), index=True)
    ...
    __table_args__ = (
        Index("ix_pe_alerts_firm_created", "firm_id", "created_at"),
    )
