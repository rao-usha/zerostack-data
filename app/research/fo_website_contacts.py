...
class FamilyOfficeWebsiteExtractor:
    ...
    async def extract_contacts_from_fo(
        self, family_office_id: int, family_office_name: str, website_url: str
    ) -> List[Dict]:
        """
    ...
    def _extract_contacts_from_html(
        self, html: str, source_url: str, family_office_id: int, family_office_name: str
    ) -> List[Dict]:
        """
        ...
        for section in team_sections:
            section_contacts = self._extract_from_team_section(
                section, source_url, family_office_id
            )
            contacts.extend(section_contacts)

        # Strategy 2: Look for general contact information
        if len(contacts) == 0:
            general_contacts = self._extract_general_contacts(
                soup, source_url, family_office_id
            )
    ...
    def _extract_from_team_section(
        self, section: BeautifulSoup, source_url: str, family_office_id: int
    ) -> List[Dict]:
        """Extract contacts from a team/people section."""
        contacts = []

        # Look for individual person cards/entries
        person_elements = section.find_all(
            ["div", "article", "li"],
            class_=re.compile(r"(person|member|profile|card|bio)", re.I),
        )
    ...
    def _extract_person_info(
        self, element: BeautifulSoup, source_url: str, family_office_id: int
    ) -> Optional[Dict]:
        ...
        try:
            ...
            # Extract title
            title = None
            for title_tag in element.find_all(
                ["p", "span", "div"], class_=re.compile(r"(title|position|role)", re.I)
            ):
            ...
            if title and is_likely_executive_title(title):
                ...
                if email_links:
                    email = (
                        email_links[0].get("href", "").replace("mailto:", "").strip()
                    )
    ...
    def _extract_general_contacts(
        self, soup: BeautifulSoup, source_url: str, family_office_id: int
    ) -> List[Dict]:
...
async def extract_contacts_for_family_offices(
    ...
    async with FamilyOfficeWebsiteExtractor(
        max_concurrency, delay_seconds
    ) as extractor:
        for fo_id, fo_name, website in family_offices:
            contacts = await extractor.extract_contacts_from_fo(
                family_office_id=fo_id, family_office_name=fo_name, website_url=website
            )
