...
async def prepare_table_for_bea_data(db: Session, dataset: str) -> Dict[str, Any]:
    """
...
async def ingest_nipa_data(
    ...
    try:
        ...
        api_response = await client.get_nipa_data(
            table_name=table_name, frequency=frequency, year=year
        )
...
async def ingest_regional_data(
    ...
    try:
        ...
        api_response = await client.get_regional_data(
            table_name=table_name, line_code=line_code, geo_fips=geo_fips, year=year
        )
        ...
        if not parsed_records:
            ...
            rows_inserted = await _insert_regional_data(
                db, db_table_name, parsed_records
            )
...
async def ingest_gdp_by_industry_data(
    ...
    try:
        ...
        api_response = await client.get_gdp_by_industry(
            table_id=table_id, frequency=frequency, year=year, industry=industry
        )
        ...
        if not parsed_records:
            ...
            rows_inserted = await _insert_gdp_industry_data(
                db, db_table_name, parsed_records
            )
...
async def ingest_international_data(
    ...
    try:
        ...
        if not parsed_records:
            ...
            rows_inserted = await _insert_international_data(
                db, db_table_name, parsed_records
            )
