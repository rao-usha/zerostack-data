...
async def prepare_table_for_dataset(db: Session, dataset: str) -> Dict[str, Any]:
    """
...
async def ingest_bls_series(
    ...
    try:
        ...
        series_summary = {series_id: len(obs) for series_id, obs in parsed_data.items()}
...
async def ingest_all_bls_datasets(
    ...
    for dataset in datasets:
        ...
        job = IngestionJob(source="bls", status=JobStatus.PENDING, config=job_config)
        db.add(job)
        db.commit()
        db.refresh(job)
...
async def ingest_unemployment_data(
    ...
    return await ingest_bls_dataset(
        db=db, job_id=job_id, dataset="cps", start_year=start_year, end_year=end_year
    )
...
async def ingest_employment_data(
    ...
    """
    return await ingest_bls_dataset(
        db=db, job_id=job_id, dataset="ces", start_year=start_year, end_year=end_year
    )
...
async def ingest_cpi_data(
    ...
    """
    return await ingest_bls_dataset(
        db=db, job_id=job_id, dataset="cpi", start_year=start_year, end_year=end_year
    )
...
async def ingest_ppi_data(
    ...
    """
    return await ingest_bls_dataset(
        db=db, job_id=job_id, dataset="ppi", start_year=start_year, end_year=end_year
    )
...
async def ingest_jolts_data(
    ...
    """
    return await ingest_bls_dataset(
        db=db, job_id=job_id, dataset="jolts", start_year=start_year, end_year=end_year
    )
