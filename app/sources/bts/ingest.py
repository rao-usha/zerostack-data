...
async def prepare_table_for_bts_data(db: Session, dataset: str) -> Dict[str, Any]:
    """
...
async def ingest_border_crossing_data(
    ...
    try:
        ...
        return {
            ...
            "filters": {"state": state, "border": border, "measure": measure},
        }
...
async def ingest_vmt_data(
    ...
    try:
        ...
        logger.info(
            f"Ingesting BTS VMT data: " f"{start_date} to {end_date}, state={state}"
        )
...
async def _insert_faf_data(
    ...
    update_set = ", ".join(
        [
            ...
            for col in columns
            if col
            not in ("year", "fr_orig", "fr_dest", "sctg2", "dms_mode", "trade_type")
        ]
    )
