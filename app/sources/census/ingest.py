...
async def prepare_table_for_acs_table(
    db: Session, survey: str, year: int, table_id: str, geo_level: str
) -> Dict[str, Any]:
    """
...
async def ingest_acs_table(
    ...
    prep_result = await prepare_table_for_acs_table(
        db, survey, year, table_id, geo_level
    )
    ...
    try:
        ...
        if all_data:
            await _batch_insert_data(db, table_name, all_data, column_mapping)
        ...
        if include_geojson:
            ...
            try:
                ...
                geojson_data = await geojson_fetcher.fetch_geojson(
                    geo_level=geo_level, state_fips=state_fips, county_fips=county_fips
                )
...
async def _store_geojson_features(
    db: Session, dataset_id: str, geo_level: str, features: List[Dict[str, Any]]
) -> None:
    """
...
async def _store_variable_metadata(
    db: Session, dataset_id: str, table_vars: Dict[str, Dict[str, Any]]
) -> None:
    """
    ...
    existing_count = (
        db.query(CensusVariableMetadata)
        .filter(CensusVariableMetadata.dataset_id == dataset_id)
        .count()
    )
