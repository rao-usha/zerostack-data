...
async def prepare_table_for_data_commons(db: Session, dataset: str) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        for i in range(0, len(places), batch_size):
            ...
            api_response = await client.bulk_observations(
                variables=[variable_dcid], entities=batch_places
            )
...
async def ingest_place_statistics(
    ...
    try:
        ...
        for i in range(0, len(variables), batch_size):
            ...
            api_response = await client.bulk_observations(
                variables=batch_vars, entities=[place_dcid]
            )
        ...
        logger.error(
            f"Data Commons place statistics ingestion failed: {e}", exc_info=True
        )
...
async def ingest_us_state_data(
    ...
    try:
        ...
        for var_dcid in variables:
            ...
            api_response = await client.bulk_observations(
                variables=[var_dcid], entities=us_state_dcids
            )
