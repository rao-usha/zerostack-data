...
async def prepare_table_for_eia_data(
    db: Session, category: str, subcategory: Optional[str] = None
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if existing:
            ...
            dataset_entry = DatasetRegistry(
                ...
                source_metadata={"category": category, "subcategory": subcategory},
            )
...
async def ingest_eia_petroleum_data(
    ...
    try:
        ...
        # Prepare table
        table_info = await prepare_table_for_eia_data(db, "petroleum", subcategory)
...
async def ingest_eia_natural_gas_data(
    ...
    try:
        ...
        table_info = await prepare_table_for_eia_data(db, "natural_gas", subcategory)
        table_name = table_info["table_name"]
...
async def ingest_eia_electricity_data(
    ...
    try:
        ...
        table_info = await prepare_table_for_eia_data(db, "electricity", subcategory)
        table_name = table_info["table_name"]
        ...
        while True:
            api_response = await client.get_electricity_data(
                route=route
                if route
                else f"electricity/{subcategory.replace('_', '-')}",
                frequency=frequency,
                start=start,
                end=end,
                facets=facets,
...
async def ingest_eia_retail_gas_prices(
    ...
    try:
        ...
        table_info = await prepare_table_for_eia_data(db, "retail_gas_prices")
        table_name = table_info["table_name"]
...
async def ingest_eia_steo_projections(
    ...
    try:
        ...
        table_info = await prepare_table_for_eia_data(db, "steo")
        table_name = table_info["table_name"]
