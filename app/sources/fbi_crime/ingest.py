...
async def ingest_fbi_crime_estimates(
    ...
    try:
        ...
        if scope == "national":
            ...
            logger.info(
                f"Fetching national estimates for {len(offenses)} offense types"
            )
            ...
            for offense in offenses:
                try:
                    ...
                    if isinstance(response, dict):
                        ...
                        logger.debug(
                            f"Response is list with {len(response)} items for {offense}"
                        )
            ...
            for state in target_states:
                for offense in offenses:
                    try:
                        ...
                        parsed = metadata.parse_state_estimates(
                            response, state, offense
                        )
                        all_rows.extend(parsed)
                    except Exception as e:
    ...
    try:
        ...
        for state in states:
            for offense in offenses:
                try:
                    logger.info(
                        f"Fetching summarized {offense} for {state} ({since}-{until})"
                    )
                    response = await client.get_summarized_data(
                        state, offense, since, until
                    )
                    parsed = metadata.parse_summarized_data(response, state, offense)
                    all_rows.extend(parsed)
                except Exception as e:
                    logger.error(
                        f"Failed to fetch summarized data for {state}/{offense}: {e}"
                    )
...
async def ingest_all_fbi_crime_data(
    db: Session, api_key: Optional[str] = None, include_states: bool = False
) -> Dict[str, Any]:
    """
    ...
    for dataset_name, ingest_func, kwargs in datasets:
        ...
        # Create job
        job_config = {"source": "fbi_crime", "dataset": dataset_name, **kwargs}
        ...
        try:
            result = await ingest_func(db=db, job_id=job.id, api_key=api_key, **kwargs)

            results[dataset_name] = {"status": "success", "job_id": job.id, **result}
...
async def _insert_estimates_data(
    db: Session, table_name: str, rows: List[Dict[str, Any]]
) -> int:
