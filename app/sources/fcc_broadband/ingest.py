...
async def prepare_table_for_fcc_data(
    db: Session,
    dataset: str
) -> Dict[str, Any]:
    """
...
async def ingest_state_coverage(
    ...
    try:
        ...
        while True:
            ...
            if parsed_batch:
                batch_inserted = await _insert_coverage_data(db, coverage_table, parsed_batch)
        ...
        if include_summary and summary_stats["record_count"] > 0:
            ...
            summary_record = {
                ...
                "avg_speed_down_mbps": summary_stats["speed_sum"] / summary_stats["record_count"] if summary_stats["record_count"] > 0 else 0,
                "broadband_coverage_pct": None,  # Would need population data
                "gigabit_coverage_pct": None,  # Would need more analysis
                "provider_competition": "high" if len(summary_stats["providers"]) > 10 else ("medium" if len(summary_stats["providers"]) > 3 else "low"),
                "data_date": None
            }
...
async def ingest_multiple_states(
    ...
    try:
        ...
        logger.info(f"Ingesting FCC broadband for {len(state_codes)} states: {state_codes}")
                ...
                try:
                    ...
                    if include_summary and summary_stats["record_count"] > 0:
                        ...
                        summary_record = {
                            ...
                            "max_speed_down_mbps": summary_stats["max_down"],
                            "max_speed_up_mbps": summary_stats["max_up"],
                            "avg_speed_down_mbps": summary_stats["speed_sum"] / summary_stats["record_count"],
                            "broadband_coverage_pct": None,
                            "gigabit_coverage_pct": None,
                            "provider_competition": "high" if len(summary_stats["providers"]) > 10 else ("medium" if len(summary_stats["providers"]) > 3 else "low"),
                            "data_date": None
                        }
                        summary_rows = await _insert_summary_data(db, summary_table, [summary_record])
                    
                    logger.info(f"Completed {state_name}: {coverage_rows} coverage rows, {summary_rows} summary rows")
...
async def _insert_coverage_data(
    ...
    columns = [
        ...
        "provider_id", "provider_name", "brand_name",
        "technology_code", "technology_name",
        "max_advertised_down_mbps", "max_advertised_up_mbps", "speed_tier",
        "business_service", "consumer_service", "data_date"
    ]
    ...
    update_cols = [
        "geography_name", "provider_name", "brand_name", "technology_name",
        "max_advertised_down_mbps", "max_advertised_up_mbps", "speed_tier",
        "business_service", "consumer_service", "data_date"
    ]
...
async def _insert_summary_data(
    ...
    columns = [
        "geography_type", "geography_id", "geography_name",
        "total_providers", "total_technologies",
        "fiber_available", "cable_available", "dsl_available",
        "fixed_wireless_available", "satellite_available", "mobile_5g_available",
        "max_speed_down_mbps", "max_speed_up_mbps", "avg_speed_down_mbps",
        "broadband_coverage_pct", "gigabit_coverage_pct",
        "provider_competition", "data_date"
    ]
    
    placeholders = ", ".join([f":{col}" for col in columns])
    column_list = ", ".join([f'"{col}"' for col in columns])
    
    update_cols = [col for col in columns if col not in ("geography_type", "geography_id")]
    update_set = ", ".join([f'"{col}" = EXCLUDED."{col}"' for col in update_cols])
