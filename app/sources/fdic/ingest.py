    ...
    try:
        ...
        if limit and limit < client.MAX_LIMIT:
            response = await client.get_bank_financials(
                cert=cert, filters=filters, limit=limit
            )
            raw_data = response.get("data", [])
        else:
            raw_data = await client.get_all_bank_financials(cert=cert, filters=filters)
...
async def ingest_institutions(
    ...
    try:
        ...
        if limit and limit < client.MAX_LIMIT:
            response = await client.get_institutions(filters=filters, limit=limit)
            raw_data = response.get("data", [])
        else:
            raw_data = await client.get_all_institutions(
                filters=filters, active_only=active_only
            )
...
async def ingest_failed_banks(
    ...
    try:
        ...
        if limit and limit < client.MAX_LIMIT:
            response = await client.get_failed_banks(filters=filters, limit=limit)
            raw_data = response.get("data", [])
        else:
    ...
    try:
        ...
        if limit and limit < client.MAX_LIMIT:
            ...
            raw_data = await client.get_all_summary_of_deposits(
                cert=cert, filters=filters, year=year
            )
...
async def ingest_all_fdic_datasets(
    ...
    for dataset_name, ingest_func in datasets_to_ingest:
        ...
        job = IngestionJob(source="fdic", status=JobStatus.PENDING, config=job_config)
        db.add(job)
        db.commit()
        db.refresh(job)
