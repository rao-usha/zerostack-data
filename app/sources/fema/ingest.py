...
async def prepare_table_for_fema_data(db: Session, dataset: str) -> Dict[str, Any]:
    """
...
async def ingest_disaster_declarations(
    ...
    try:
        ...
        records = await client.get_all_disaster_declarations(
            state=state, year=year, disaster_type=disaster_type, max_records=max_records
        )
        ...
        if not parsed_records:
            ...
            rows_inserted = await _insert_disaster_declarations(
        ...
        return {
            ...
            "filters": {"state": state, "year": year, "disaster_type": disaster_type},
        }
    ...
    try:
        ...
        while len(all_records) < max_records:
            response = await client.get_public_assistance_projects(
                skip=skip, top=top, state=state, disaster_number=disaster_number
            )
            ...
            logger.info(
                f"Fetched {len(records)} PA records (total: {len(all_records)})"
            )
    ...
    try:
        ...
        while len(all_records) < max_records:
            response = await client.get_hazard_mitigation_projects(
                skip=skip, top=top, state=state, program_area=program_area
            )
            ...
            all_records.extend(records)
            logger.info(
                f"Fetched {len(records)} HMA records (total: {len(all_records)})"
            )
...
async def _insert_hma_projects(
    ...
    column_list = ", ".join(columns)
    update_set = ", ".join(
        [f"{col} = EXCLUDED.{col}" for col in columns if col != "project_identifier"]
    )
