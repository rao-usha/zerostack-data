...
class WorldBankClient:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized WorldBankClient: max_concurrency={max_concurrency}"
        )
    ...
    async def _request_with_retry(
        ...
        async with self.semaphore:  # Bounded concurrency
            ...
            raise Exception(f"Failed to fetch World Bank data after {self.max_retries} attempts")
...
class IMFClient:
    ...
    async def _request_with_retry(
        ...
        async with self.semaphore:
            ...
            raise Exception(f"Failed to fetch IMF data after {self.max_retries} attempts")
...
class OECDClient:
    ...
    async def get_data(
        ...
        if not dataflow:
            raise ValueError(f"Unknown dataset: {dataset}. Available: {list(self.DATAFLOWS.keys())}")
    ...
    def _parse_sdmx_json(
        ...
        try:
            ...
            for dim in dimensions:
                dim_id = dim.get("id")
                values = {str(i): v.get("id") for i, v in enumerate(dim.get("values", []))}
    ...
    def _parse_sdmx_json_new(
        ...
        try:
            ...
            for dim in series_dimensions:
                ...
                series_dim_lookup[dim_id] = {str(i): v.get("id", v.get("name", str(i))) for i, v in enumerate(values_list)}
            ...
            for dim in obs_dimensions:
                dim_id = dim.get("id", "")
                values_list = dim.get("values", [])
                obs_dim_lookup[dim_id] = {str(i): v.get("id", v.get("name", str(i))) for i, v in enumerate(values_list)}
    ...
    async def get_key_economic_indicators(
        ...
        try:
            ...
            filtered_data = [
                ...
                if record.get("ref_area", "") in countries
            ]
    ...
    async def get_labour_force_statistics(
        ...
        try:
            ...
            logger.info(f"Filtered to {len(filtered_data)} labor records for {countries}")
            return filtered_data
    ...
    async def get_trade_in_services(
        ...
        try:
            ...
            logger.info(f"Filtered to {len(filtered_data)} trade records for {countries}")
            return filtered_data
    ...
    async def get_tax_revenue_statistics(
        ...
        try:
            ...
            filtered_data = [
                record for record in data
                if record.get("cou", "") in countries or record.get("ref_area", "") in countries
            ]
    ...
    async def _request_with_retry(
        ...
        async with self.semaphore:
            ...
            for attempt in range(self.max_retries):
                try:
                    ...
                    if e.response.status_code >= 500 and attempt < self.max_retries - 1:
                        ...
                        raise Exception(f"OECD API HTTP error: {e.response.status_code}")
            ...
            raise Exception(f"Failed to fetch OECD data after {self.max_retries} attempts")
...
class BISClient:
        ...
        try:
            ...
            for series_key, series_data in series.items():
                ...
                for i, idx in enumerate(series_indices):
                    if i < len(dimensions):
                        ...
                        if int(idx) < len(values):
                            series_attrs[dim.get("id", f"dim_{i}").lower()] = values[int(idx)].get("id")
                ...
                for obs_key, obs_value in observations.items():
                    ...
                    time_period = time_values[time_idx].get("id") if time_idx < len(time_values) else obs_key
    ...
    async def _request_with_retry(
        ...
        async with self.semaphore:
            ...
            raise Exception(f"Failed to fetch BIS data after {self.max_retries} attempts")
