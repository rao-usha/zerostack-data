...
async def ingest_worldbank_wdi(
    ...
    try:
        ...
        for indicator in indicators:
            indicator_data = [d for d in all_data if d and d.get("indicator", {}).get("id") == indicator]
            parsed = metadata.parse_worldbank_data(indicator_data, indicator)
            all_parsed.extend(parsed)
        
        # If indicators weren't separated, parse all together
        if not all_parsed:
            all_parsed = metadata.parse_worldbank_data(all_data, indicators[0] if indicators else "")
        ...
        if all_parsed:
            ...
            columns = [
                ...
                "country_iso3", "year", "value", "unit", "decimal_places"
            ]
...
async def ingest_worldbank_countries(
    db: Session,
    job_id: int
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if parsed:
            columns = [
                "country_id", "country_name", "iso3_code", "iso2_code",
...
async def ingest_imf_weo(
    ...
    try:
        ...
        if job:
            ...
            job.completed_at = datetime.utcnow()
            job.rows_inserted = rows_inserted
            job.error_message = "IMF WEO ingestion requires specific endpoint configuration"
            db.commit()
...
async def ingest_imf_ifs(
    ...
    try:
        ...
        logger.info(f"Ingesting IMF IFS data: indicator={indicator}, {start_year}-{end_year}")
        ...
        if parsed:
            columns = [
                "indicator_code", "country_code", "period",
                "frequency", "value", "unit_mult", "status"
            ]
...
async def ingest_oecd_mei(
    ...
    try:
        ...
        if parsed:
            columns = [
                "indicator_code", "country_code", "subject", "measure",
                "frequency", "period", "value", "unit", "powercode"
            ]
...
async def ingest_oecd_kei(
    ...
    try:
        ...
        if data:
            columns = [
                "indicator_code", "country_code", "subject", "measure",
                "frequency", "period", "value", "unit", "transformation"
            ]
            ...
            for record in data:
                row = {
                    ...
                    "value": float(record.get("value")) if record.get("value") is not None else None,
...
async def ingest_oecd_labor(
    ...
    try:
        ...
        if data:
            columns = [
                "country_code", "indicator_code", "sex", "age",
                "frequency", "period", "value", "unit_measure"
            ]
...
async def ingest_oecd_trade(
    ...
    try:
        ...
        if data:
            columns = [
                "reporter_country", "partner_country", "flow", "service_item",
                "frequency", "period", "value", "unit_measure", "currency"
            ]
            ...
            for record in data:
                row = {
                    "reporter_country": record.get("ref_area") or record.get("reporter"),
                    "partner_country": record.get("counterpart_area") or record.get("partner"),
...
async def ingest_oecd_tax(
    ...
    try:
        ...
        if data:
            columns = [
                "country_code", "tax_type", "government_level",
                "measure", "period", "value", "unit"
            ]
...
async def ingest_bis_eer(
    ...
    try:
        ...
        logger.info(f"Ingesting BIS EER data: type={eer_type}, {start_period}-{end_period}")
...
async def ingest_bis_property_prices(
    ...
    try:
        ...
        if parsed:
            columns = [
                "country_code", "property_type", "unit_measure",
                "period", "frequency", "value"
            ]
