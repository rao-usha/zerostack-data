...
class IRSSOIClient:
        ...
        async with self.semaphore:  # Bounded concurrency
            ...
            for attempt in range(self.max_retries):
                try:
                    ...
                    logger.warning(f"HTTP error downloading {url} (attempt {attempt + 1}): {e}")
            ...
            raise Exception(f"Failed to download {url} after {self.max_retries} attempts")
    ...
    async def get_migration_data(
        ...
        if flow_type == "inflow":
            ...
            raise ValueError(f"Invalid flow_type: {flow_type}. Must be 'inflow' or 'outflow'")
