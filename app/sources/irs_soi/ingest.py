...
async def prepare_table_for_soi_data(db: Session, dataset: str) -> Dict[str, Any]:
    """
...
async def ingest_zip_income_data(
    db: Session, job_id: int, year: Optional[int] = None, use_cache: bool = True
) -> Dict[str, Any]:
    """
...
async def ingest_migration_data(
    ...
    try:
        ...
        logger.info(
            f"Ingesting IRS SOI migration data for year {year}, flow_type={flow_type}"
        )
        ...
        for ft in flow_types:
            df = await client.get_migration_data(
                year=year, flow_type=ft, use_cache=use_cache
            )
        ...
        for dataset_name, ingest_func in datasets:
            try:
                ...
                if dataset_name == "migration":
                    ...
                    result = await ingest_func(
                        db=db, job_id=sub_job.id, year=year, use_cache=use_cache
                    )
...
async def _insert_records_batch(
    db: Session, table_name: str, records: List[Dict[str, Any]], dataset: str
) -> int:
    """
