...
class KaggleClient:
    ...
    def _download_competition_files_sync(
        ...
        try:
            ...
            file_names_list = [f.name if hasattr(f, 'name') else str(f) for f in available_files]
            logger.info(f"Available files: {file_names_list}")
            
            # Determine which files to download
            if file_names:
                files_to_download = [f for f in available_files if (f.name if hasattr(f, 'name') else str(f)) in file_names]
            ...
            for file_info in files_to_download:
                ...
                if downloaded_path.suffix == '.zip' or (comp_dir / f"{file_name}.zip").exists():
                    zip_path = comp_dir / f"{file_name}.zip" if (comp_dir / f"{file_name}.zip").exists() else downloaded_path
                    if zip_path.exists():
    ...
    async def download_m5_files(self, force: bool = False) -> Dict[str, Path]:
        ...
        return await self.download_competition_files(
            competition=self.M5_COMPETITION,
            file_names=m5_files,
            force=force
        )
