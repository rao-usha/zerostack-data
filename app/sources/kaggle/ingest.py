        ...
        for row in reader:
            ...
            if len(sales_batch) >= batch_size:
                ...
                if sales_inserted % 500000 == 0:
                    logger.info(f"Inserted {sales_inserted} sales rows, {items_inserted} items...")
...
async def ingest_m5_dataset(
    ...
    try:
        ...
        calendar_path = client.get_local_file_path(KaggleClient.M5_COMPETITION, "calendar.csv")
        prices_path = client.get_local_file_path(KaggleClient.M5_COMPETITION, "sell_prices.csv")
        sales_path = client.get_local_file_path(KaggleClient.M5_COMPETITION, "sales_train_validation.csv")
        ...
        if not sales_path or not sales_path.exists():
            raise FileNotFoundError("sales_train_validation.csv not found after download")
