...
class NOAAClient(BaseAPIClient):
    ...
    async def get_datasets(self) -> List[Dict[str, Any]]:
        ...
        result = await self.get("datasets", params={"limit": 1000}, resource_id="datasets")
    ...
    async def get_locations(
        ...
        result = await self.get("locations", params=params, resource_id=f"locations:{dataset_id}")
        return result.get("results", [])
    ...
    async def get_stations(
        ...
        result = await self.get("stations", params=params, resource_id=f"stations:{dataset_id}")
        return result.get("results", [])
    ...
    async def get_all_data_paginated(
        ...
        while True:
            ...
            offset += len(results)
            logger.info(f"Fetched {len(all_data)} records so far (offset={offset}, total={count})")
