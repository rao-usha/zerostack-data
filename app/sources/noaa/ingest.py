...
async def create_noaa_table(session: AsyncSession, dataset_key: str) -> None:
    """
...
async def create_noaa_indexes(session: AsyncSession, table_name: str) -> None:
    """
            ...
            try:
                ...
                if data:
                    rows_inserted = await insert_noaa_data(
                        session=session, table_name=dataset.table_name, data=data
                    )

                    logger.info(
                        f"Inserted {rows_inserted} rows into {dataset.table_name}"
                    )
                else:
                    rows_inserted = 0
...
async def update_job_status(
    ...
    await session.execute(
        text("""
            ...
            WHERE id = :job_id
        """),
        {"job_id": job_id, "status": status, "result": str(result)},
    )
    await session.commit()


async def insert_noaa_data(
    session: AsyncSession, table_name: str, data: List[Dict[str, Any]]
) -> int:
    """
