...
class KalshiClient:
    ...
    async def _get_client(self) -> httpx.AsyncClient:
        ...
        if self._client is None or self._client.is_closed:
            self._client = httpx.AsyncClient(
                timeout=self.timeout, headers={"User-Agent": "NexdataResearch/1.0"}
            )
        return self._client
    ...
    async def _request(
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                logger.warning(
                    f"Kalshi API error (attempt {attempt + 1}): {e.response.status_code}"
                )
    ...
    async def get_events(
        self, series_ticker: Optional[str] = None, status: str = "open"
    ) -> Dict:
        """
        ...
        if categories:
            ...
            for series in categories:
                try:
                    ...
                    logger.info(
                        f"Kalshi: Fetched {len(markets)} markets from series {series}"
                    )
...
class PolymarketClient:
    ...
    async def _request(
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                logger.warning(
                    f"Polymarket API error (attempt {attempt + 1}): {e.response.status_code}"
                )
                if attempt == self.max_retries - 1:
                    raise
                await asyncio.sleep(2**attempt)
    ...
    def _parse_market(
        ...
        try:
            question = (
                raw.get("question") or raw.get("groupItemTitle") or raw.get("title", "")
            )
            if not question:
                return None
            ...
            slug = raw.get("slug") or (
                event_context.get("slug") if event_context else None
            )
