...
def upsert_market(db: Session, market_data: MarketData) -> Tuple[PredictionMarket, bool]:
    """
    ...
    if existing:
        ...
        existing.last_volume_usd = str(market_data.volume_usd) if market_data.volume_usd else None
        ...
        market = PredictionMarket(
            ...
            last_volume_usd=str(market_data.volume_usd) if market_data.volume_usd else None,
...
def store_observation(db: Session, market: PredictionMarket, market_data: MarketData) -> MarketObservation:
    """Store a new market observation."""
    # Get previous observation for calculating changes
    prev_obs = db.query(MarketObservation).filter(
    ...
    observation = MarketObservation(
        ...
        no_probability=str(market_data.no_probability) if market_data.no_probability else None,
        volume_usd=str(market_data.volume_usd) if market_data.volume_usd else None,
        volume_24h_usd=str(market_data.volume_24h_usd) if market_data.volume_24h_usd else None,
        liquidity_usd=str(market_data.liquidity_usd) if market_data.liquidity_usd else None,
...
def check_and_generate_alerts(
    ...
    recent_obs = db.query(MarketObservation).filter(
        MarketObservation.market_id == market.id,
        MarketObservation.observation_timestamp >= datetime.utcnow() - timedelta(hours=24)
    ).order_by(MarketObservation.observation_timestamp.asc()).first()
    ...
    try:
        ...
        if change >= threshold:
            ...
            logger.info(f"Generated {severity} alert for market {market.id}: {alert_message}")
            return alert
    ...
    try:
        ...
        for market_data in markets:
            try:
                ...
                logger.error(f"Error processing Kalshi market {market_data.market_id}: {e}")
    ...
    try:
        ...
        for market_data in markets:
            try:
                ...
                logger.error(f"Error processing Polymarket market {market_data.market_id}: {e}")
...
async def monitor_all_platforms(
    ...
    try:
        # Monitor Kalshi
        try:
            kalshi_results = await monitor_kalshi(db, kalshi_categories, limit_per_platform)
        ...
        complete_job(
            ...
            markets_updated=results["totals"]["markets_checked"] - results["totals"]["new_markets"],
    ...
    return [
        {
            ...
            "yes_probability": float(m.last_yes_probability) if m.last_yes_probability else None,
...
def get_market_history(
    ...
    ).order_by(MarketObservation.observation_timestamp.asc()).all()
...
def get_active_alerts(
    ...
    result = []
    for a in alerts:
        market = db.query(PredictionMarket).filter(PredictionMarket.id == a.market_id).first()
        result.append({
            "id": a.id,
            ...
            "probability_before": float(a.probability_before) if a.probability_before else None,
            "probability_after": float(a.probability_after) if a.probability_after else None,
            "probability_change": float(a.probability_change) if a.probability_change else None,
...
def get_dashboard_data(db: Session) -> Dict:
    ...
    for platform in ["kalshi", "polymarket"]:
        count = db.query(PredictionMarket).filter(
    ...
    recent_alerts = db.query(MarketAlert).filter(
        MarketAlert.triggered_at >= datetime.utcnow() - timedelta(hours=24)
    ).count()
    
    # Unacknowledged alerts
    unacked_alerts = db.query(MarketAlert).filter(
        MarketAlert.is_acknowledged == 0
    ).count()
...
def get_top_probability_movers(
    ...
    ).order_by(MarketObservation.observation_timestamp.desc()).all()
    ...
    for obs in sorted_obs:
        market = db.query(PredictionMarket).filter(PredictionMarket.id == obs.market_id).first()
        if market:
            result.append({
                "market_id": market.id,
                ...
                "yes_probability": float(obs.yes_probability) if obs.yes_probability else None,
                "probability_change_24h": float(obs.probability_change_24h) if obs.probability_change_24h else None,
