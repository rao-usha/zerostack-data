...
def store_document_text_sections(
    ...
    logger.info(f"Stored {len(created_sections)} text sections for document_id={document_id}")
...
def upsert_strategy_snapshot(
    ...
    existing = db.query(LpStrategySnapshot).filter(
...
def upsert_asset_class_allocations(
    ...
    for alloc_input in allocations:
        ...
        existing = db.query(LpAssetClassTargetAllocation).filter(
        ...
        if existing:
            # Update existing
            existing.target_weight_pct = str(alloc_input.target_weight_pct) if alloc_input.target_weight_pct is not None else None
            existing.min_weight_pct = str(alloc_input.min_weight_pct) if alloc_input.min_weight_pct is not None else None
            existing.max_weight_pct = str(alloc_input.max_weight_pct) if alloc_input.max_weight_pct is not None else None
            existing.current_weight_pct = str(alloc_input.current_weight_pct) if alloc_input.current_weight_pct is not None else None
            existing.benchmark_weight_pct = str(alloc_input.benchmark_weight_pct) if alloc_input.benchmark_weight_pct is not None else None
            ...
            allocation = LpAssetClassTargetAllocation(
                ...
                target_weight_pct=str(alloc_input.target_weight_pct) if alloc_input.target_weight_pct is not None else None,
                min_weight_pct=str(alloc_input.min_weight_pct) if alloc_input.min_weight_pct is not None else None,
                max_weight_pct=str(alloc_input.max_weight_pct) if alloc_input.max_weight_pct is not None else None,
                current_weight_pct=str(alloc_input.current_weight_pct) if alloc_input.current_weight_pct is not None else None,
                benchmark_weight_pct=str(alloc_input.benchmark_weight_pct) if alloc_input.benchmark_weight_pct is not None else None,
    ...
    logger.info(f"Upserted {len(results)} asset class allocations for strategy_id={strategy_id}")
    return results
...
def upsert_asset_class_projections(
    ...
    for proj_input in projections:
        ...
        existing = db.query(LpAssetClassProjection).filter(
            LpAssetClassProjection.strategy_id == strategy_id,
            LpAssetClassProjection.asset_class == proj_input.asset_class,
            LpAssetClassProjection.projection_horizon == proj_input.projection_horizon,
        ).first()
        
        if existing:
            # Update existing
            existing.net_flow_projection_amount = str(proj_input.net_flow_projection_amount) if proj_input.net_flow_projection_amount is not None else None
            existing.commitment_plan_amount = str(proj_input.commitment_plan_amount) if proj_input.commitment_plan_amount is not None else None
            existing.expected_return_pct = str(proj_input.expected_return_pct) if proj_input.expected_return_pct is not None else None
            existing.expected_volatility_pct = str(proj_input.expected_volatility_pct) if proj_input.expected_volatility_pct is not None else None
            ...
            projection = LpAssetClassProjection(
                ...
                net_flow_projection_amount=str(proj_input.net_flow_projection_amount) if proj_input.net_flow_projection_amount is not None else None,
                commitment_plan_amount=str(proj_input.commitment_plan_amount) if proj_input.commitment_plan_amount is not None else None,
                expected_return_pct=str(proj_input.expected_return_pct) if proj_input.expected_return_pct is not None else None,
                expected_volatility_pct=str(proj_input.expected_volatility_pct) if proj_input.expected_volatility_pct is not None else None,
    ...
    logger.info(f"Upserted {len(results)} asset class projections for strategy_id={strategy_id}")
    return results
...
def upsert_thematic_tags(
    ...
    for tag_input in tags:
        ...
        existing = db.query(LpStrategyThematicTag).filter(
        ...
        if existing:
            # Update existing
            existing.relevance_score = str(tag_input.relevance_score) if tag_input.relevance_score is not None else None
            ...
            tag = LpStrategyThematicTag(
                ...
                relevance_score=str(tag_input.relevance_score) if tag_input.relevance_score is not None else None,
...
def ingest_lp_strategy_document(
    ...
    if not fund_config:
        raise ValueError(f"Unknown LP fund: {lp_name}. Must be in KNOWN_LP_FUNDS or pre-registered.")
