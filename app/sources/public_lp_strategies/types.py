...
class LpFundInput(BaseModel):
    ...
    name: str = Field(
        ..., min_length=1, max_length=255, description="Short LP name (e.g., 'CalPERS')"
    )
    formal_name: Optional[str] = Field(None, description="Full formal name")
    lp_type: str = Field(
        ..., description="LP type: 'public_pension', 'sovereign_wealth', 'endowment'"
    )
    jurisdiction: Optional[str] = Field(
        None, max_length=100, description="Jurisdiction (e.g., 'CA', 'NY')"
    )
...
class LpDocumentInput(BaseModel):
    ...
    document_type: str = Field(
        ..., description="Document type (e.g., 'investment_committee_presentation')"
    )
    program: str = Field(
        ..., description="Program/portfolio (e.g., 'private_equity', 'total_fund')"
    )

    report_period_start: Optional[date] = Field(
        None, description="Report period start date"
    )
    report_period_end: Optional[date] = Field(
        None, description="Report period end date"
    )
    fiscal_year: Optional[int] = Field(
        None, ge=1900, le=2100, description="Fiscal year"
    )
    fiscal_quarter: Optional[str] = Field(
        None, description="Fiscal quarter: 'Q1', 'Q2', 'Q3', 'Q4'"
    )

    source_url: str = Field(..., description="Source URL of the document")
    file_format: str = Field(
        ..., description="File format: 'pdf', 'pptx', 'html', etc."
    )
    raw_file_location: Optional[str] = Field(
        None, description="S3 path or blob identifier"
    )
    ...
    def validate_document_type(cls, v: str) -> str:
        ...
        if v not in VALID_DOCUMENT_TYPES:
            raise ValueError(
                f"Invalid document_type. Must be one of: {VALID_DOCUMENT_TYPES}"
            )
        return v
    ...
    def validate_fiscal_quarter(cls, v: Optional[str]) -> Optional[str]:
        ...
        if v not in VALID_FISCAL_QUARTERS:
            raise ValueError(
                f"Invalid fiscal_quarter. Must be one of: {VALID_FISCAL_QUARTERS}"
            )
        return v
    ...
    def validate_file_format(cls, v: str) -> str:
        ...
        if v not in VALID_FILE_FORMATS:
            raise ValueError(
                f"Invalid file_format. Must be one of: {VALID_FILE_FORMATS}"
            )
        return v
...
class AssetClassAllocationInput(BaseModel):
    ...
    min_weight_pct: Optional[float] = Field(
        None, description="Minimum allocation percentage"
    )
    max_weight_pct: Optional[float] = Field(
        None, description="Maximum allocation percentage"
    )
    current_weight_pct: Optional[float] = Field(
        None, description="Current allocation percentage"
    )
    benchmark_weight_pct: Optional[float] = Field(
        None, description="Benchmark allocation percentage"
    )
    source_section_id: Optional[int] = Field(
        None, description="FK to source text section"
    )
    ...
    def validate_asset_class(cls, v: str) -> str:
        ...
        if v not in VALID_ASSET_CLASSES:
            raise ValueError(
                f"Invalid asset_class. Must be one of: {VALID_ASSET_CLASSES}"
            )
        return v
...
class AssetClassProjectionInput(BaseModel):
    ...
    projection_horizon: str = Field(
        ..., description="Projection horizon (e.g., '3_year')"
    )
    net_flow_projection_amount: Optional[float] = Field(
        None, description="Net flow projection (currency)"
    )
    commitment_plan_amount: Optional[float] = Field(
        None, description="Commitment plan amount"
    )
    ...
    expected_volatility_pct: Optional[float] = Field(
        None, description="Expected volatility percentage"
    )
    source_section_id: Optional[int] = Field(
        None, description="FK to source text section"
    )
    ...
    def validate_horizon(cls, v: str) -> str:
        ...
        if v not in VALID_HORIZONS:
            raise ValueError(
                f"Invalid projection_horizon. Must be one of: {VALID_HORIZONS}"
            )
        return v
...
class ThematicTagInput(BaseModel):
    ...
    def validate_theme(cls, v: str) -> str:
        ...
        if v not in VALID_THEMES:
            ...
            logger.warning(
                f"Using custom theme '{v}' not in standard list: {VALID_THEMES}"
            )
        return v
...
class StrategySnapshotInput(BaseModel):
    ...
    strategy_date: Optional[date] = Field(
        None, description="Strategy date (board/IC date)"
    )
    primary_document_id: Optional[int] = Field(
        None, description="FK to primary document"
    )
...
class StrategyExtractionResult(BaseModel):
    ...
    allocations: List[AssetClassAllocationInput] = Field(
        default_factory=list, description="Asset class allocations"
    )
    projections: List[AssetClassProjectionInput] = Field(
        default_factory=list, description="Asset class projections"
    )
    thematic_tags: List[ThematicTagInput] = Field(
        default_factory=list, description="Thematic tags"
    )

    # Optional: manager/vehicle exposures (not always available)
    manager_exposures: List[Dict[str, Any]] = Field(
        default_factory=list, description="Manager/vehicle exposures"
    )
...
class LpKeyContactInput(BaseModel):
    ...
    lp_id: int = Field(..., description="Foreign key to lp_fund.id")
    full_name: str = Field(
        ..., min_length=2, max_length=255, description="Full name of contact"
    )
    title: Optional[str] = Field(
        None, description="Job title (e.g., 'Chief Investment Officer')"
    )
    role_category: Optional[str] = Field(
        None, description="Standardized role: CIO, CFO, CEO, Investment Director, etc."
    )

    # Contact information (only if publicly disclosed)
    email: Optional[str] = Field(
        None, description="Professional email (only if publicly listed)"
    )
    phone: Optional[str] = Field(
        None, description="Phone number (only if publicly listed)"
    )
    linkedin_url: Optional[str] = Field(
        None, description="LinkedIn URL for manual reference (no scraping)"
    )

    # Data provenance
    source_document_id: Optional[int] = Field(
        None, description="FK to lp_document.id if from document"
    )
    source_type: Optional[str] = Field(
        None,
        description="Source: sec_adv, website, disclosure_doc, annual_report, manual",
    )
    source_url: Optional[str] = Field(
        None, description="URL where information was found"
    )
    confidence_level: Optional[str] = Field(
        "medium", description="Quality indicator: high, medium, low"
    )
    is_verified: int = Field(
        0, ge=0, le=1, description="1 if manually verified, 0 otherwise"
    )
    ...
    def validate_email(cls, v: Optional[str]) -> Optional[str]:
        ...
        if email_prefix in generic_prefixes:
            ...
            if not any(
                term in v.lower() for term in ["investment", "investor", "relations"]
            ):
    ...
    def validate_confidence_level(cls, v: Optional[str]) -> Optional[str]:
        ...
        if v not in valid_levels:
            raise ValueError(
                f"Invalid confidence_level. Must be one of: {valid_levels}"
            )
        return v
