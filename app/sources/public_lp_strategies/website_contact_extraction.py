...
def find_contact_page_links(soup: BeautifulSoup, base_url: str) -> List[str]:
    ...
    for link in soup.find_all("a", href=True):
        ...
        if any(
            keyword in href.lower() or keyword in text for keyword in target_keywords
        ):
...
def extract_executives(soup: BeautifulSoup) -> List[Dict[str, Any]]:
    ...
    title_patterns = [
        ...
        (
            r"\bHead of (?:Investments?|Private Equity|Alternatives)\b",
            "Investment Director",
        ),
...
async def crawl_lp_website(homepage_url: str, lp_name: str) -> Dict[str, Any]:
    """
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            logger.info(
                f"Found {len(contact_links)} contact-related pages for {lp_name}"
            )
...
async def crawl_multiple_lps(lp_list: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
...
def assemble_contacts_from_extraction(
    extraction_result: Dict[str, Any], lp_id: int
) -> List[Dict[str, Any]]:
    """
    ...
    for contact in contacts:
        contact["confidence_level"] = calculate_confidence_score(
            contact.get("email"), contact.get("phone"), "website", False
        )
