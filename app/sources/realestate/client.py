    ...
    async def fetch_house_price_index(
        ...
        async with self.semaphore:
            ...
            for attempt in range(self.max_retries):
                try:
                    logger.info(f"Fetching FHFA HPI data (attempt {attempt+1}/{self.max_retries})")
...
class HUDClient:
    ...
    async def fetch_permits_and_starts(
        ...
        async with self.semaphore:
            ...
            for attempt in range(self.max_retries):
                try:
                    logger.info(f"Fetching HUD permits data (attempt {attempt+1}/{self.max_retries})")
                    
                    # Build API URL based on geography type
                    url = f"{self.BASE_URL}/buildingpermits"
                    
                    params = {
                        "type": geography_type.lower()
                    }
...
class RedfinClient:
    ...
    async def fetch_redfin_data(
        ...
        async with self.semaphore:
            ...
            for attempt in range(self.max_retries):
                try:
                    logger.info(f"Fetching Redfin data for {region_type} (attempt {attempt+1}/{self.max_retries})")
...
class OSMClient:
    ...
    async def _get_client(self) -> httpx.AsyncClient:
        ...
        if self._client is None:
            self._client = httpx.AsyncClient(
                timeout=httpx.Timeout(180.0, connect=10.0),  # Overpass queries can be slow
                follow_redirects=True
            )
        return self._client
    ...
    async def fetch_buildings(
        ...
        async with self.semaphore:
            ...
            for attempt in range(self.max_retries):
                try:
                    logger.info(f"Fetching OSM buildings (attempt {attempt+1}/{self.max_retries})")
