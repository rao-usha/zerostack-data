...
def parse_adviser_info(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        business_addr = (
            info.get("business_address") or info.get("businessAddress") or {}
        )
        ...
        # Extract AUM info
        aum_info = (
            info.get("assets_under_management")
            or info.get("assetsUnderManagement")
            or {}
        )
        ...
        parsed = {
            ...
            "doing_business_as": info.get("doing_business_as")
            or info.get("doingBusinessAs"),
            # Business address
            "business_address_street1": business_addr.get("street1")
            or business_addr.get("street_1"),
            "business_address_street2": business_addr.get("street2")
            or business_addr.get("street_2"),
            "business_address_city": business_addr.get("city"),
            "business_address_state": business_addr.get("state"),
            "business_address_zip": business_addr.get("zip")
            or business_addr.get("postal_code"),
            ...
            # Mailing address (if different)
            "mailing_address_street1": mailing_addr.get("street1")
            or mailing_addr.get("street_1"),
            "mailing_address_street2": mailing_addr.get("street2")
            or mailing_addr.get("street_2"),
            "mailing_address_city": mailing_addr.get("city"),
            "mailing_address_state": mailing_addr.get("state"),
            "mailing_address_zip": mailing_addr.get("zip")
            or mailing_addr.get("postal_code"),
            ...
            "pooled_investment_vehicle_count": _parse_int(
                clients.get("pooled_vehicles")
            ),
            # Business type
...
def parse_key_personnel(
    crd_number: str, personnel_data: List[Dict[str, Any]]
) -> List[Dict[str, Any]]:
    """
    ...
    for person in personnel_data:
        try:
            parsed = {
                "crd_number": crd_number,
                "individual_crd_number": person.get("crd_number")
                or person.get("crdNumber"),
                ...
                "position_type": person.get("position_type")
                or person.get("positionType"),
...
def _is_family_office(info: Dict[str, Any]) -> bool:
    ...
    # Check business type
    business_type = (
        info.get("business_type") or info.get("businessType") or ""
    ).lower()
