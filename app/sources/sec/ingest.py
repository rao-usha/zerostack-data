    ...
    try:
        ...
        if existing:
            ...
            dataset_entry = DatasetRegistry(
                ...
                description=metadata.get_filing_type_description(filing_type),
                source_metadata={"filing_type": filing_type},
            )
...
async def ingest_company_filings(
    ...
    try:
        ...
        for filing_type, filings in filings_by_type.items():
            ...
            logger.info(
                f"Inserting {len(filings)} {filing_type} filings into {table_name}"
            )
...
async def ingest_multiple_companies(
    ...
    for cik in ciks:
        ...
        job = IngestionJob(source="sec", status=JobStatus.PENDING, config=job_config)
        db.add(job)
        db.commit()
        db.refresh(job)
        ...
        try:
            ...
            results[cik] = {"status": "success", "job_id": job.id, **result}

        except Exception as e:
            logger.error(f"Failed to ingest CIK {cik}: {e}")
            results[cik] = {"status": "failed", "job_id": job.id, "error": str(e)}
