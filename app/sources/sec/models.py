...
class SECFinancialFact(Base):
    ...
    # Fact metadata
    fact_name = Column(Text, nullable=False, index=True)  # e.g., "Assets", "Revenues", "NetIncomeLoss"
...
class SECFilingSection(Base):
    ...
    # Section metadata
    section_number = Column(String(10), nullable=True, index=True)  # e.g., "1", "1A", "7", "8"
    section_name = Column(Text, nullable=False, index=True)  # e.g., "Business", "Risk Factors"
    ...
    __table_args__ = (
        ...
        Index('idx_sec_sections_accession_section', 'accession_number', 'section_number'),
    )
