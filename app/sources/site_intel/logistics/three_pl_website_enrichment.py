...
ABOUT_PAGE_PATHS = [
    ...
    "/about/overview", "/company/about", "/about/company",
]
...
EMPLOYEE_PATTERNS = [
    re.compile(r"([\d,]+)\s*(?:\+\s*)?(?:employees|team members|associates|workers|people)", re.IGNORECASE),
    re.compile(r"(?:more than|over|approximately|nearly)\s*([\d,]+)\s*(?:employees|team members|associates)", re.IGNORECASE),
    re.compile(r"(?:workforce|staff|team)\s+(?:of\s+)?(?:more than\s+|over\s+)?([\d,]+)", re.IGNORECASE),
]

HQ_PATTERNS = [
    re.compile(r"(?:headquartered|based|located)\s+in\s+([A-Z][a-z]+(?:\s[A-Z][a-z]+)*),?\s+([A-Z]{2})", re.IGNORECASE),
    re.compile(r"(?:headquarters|head office|corporate office|main office)\s*(?:in|:)?\s*([A-Z][a-z]+(?:\s[A-Z][a-z]+)*),?\s+([A-Z]{2})", re.IGNORECASE),
]

FACILITY_PATTERNS = [
    re.compile(r"([\d,]+)\s*(?:\+\s*)?(?:facilities|locations|warehouses|offices|terminals|service centers|distribution centers)", re.IGNORECASE),
    re.compile(r"(?:more than|over|approximately)\s*([\d,]+)\s*(?:facilities|locations|warehouses)", re.IGNORECASE),
    re.compile(r"(?:network of|operates)\s*([\d,]+)\s*(?:facilities|locations|warehouses)", re.IGNORECASE),
]
...
class ThreePLWebsiteEnrichmentCollector(BaseCollector):
    ...
    async def collect(self, config: CollectionConfig) -> CollectionResult:
        ...
        try:
            ...
            companies = self.db.query(ThreePLCompany).filter(
            ...
            async with httpx.AsyncClient(
                ...
                for company in companies:
                    ...
                    try:
                        ...
                        logger.debug(f"Website scrape failed for {company.company_name}: {e}")
                        errors.append({
    ...
    async def _scrape_company(
        ...
        data_fields = {k: v for k, v in record.items() if k != "company_name" and v is not None}
        if not data_fields:
            return None
