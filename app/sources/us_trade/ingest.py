...
async def prepare_table_for_trade_data(
    db: Session,
    dataset: str
) -> Dict[str, Any]:
    """
...
async def _insert_exports_hs_data(
    ...
    columns = [
        "year", "month", "country_code", "country_name",
        "hs_code", "commodity_desc", "value_monthly", "value_ytd",
        "quantity_monthly", "quantity_ytd", "quantity_unit"
    ]
...
async def _insert_imports_hs_data(
    ...
    columns = [
        "year", "month", "country_code", "country_name",
        "hs_code", "commodity_desc", "general_value_monthly", "general_value_ytd",
        "consumption_value_monthly", "consumption_value_ytd",
        "dutyfree_value_monthly", "dutiable_value_monthly",
        "quantity_monthly", "quantity_ytd", "quantity_unit"
    ]
...
async def _insert_state_exports_data(
    ...
    columns = [
        "year", "month", "state_code", "state_name",
        "country_code", "country_name", "hs_code", "commodity_desc",
        "value_monthly", "value_ytd"
    ]
...
async def _insert_port_trade_data(
    ...
    columns = [
        "year", "month", "district_code", "district_name",
        "country_code", "country_name", "hs_code", "commodity_desc",
        "trade_type", "value_monthly", "value_ytd"
    ]
    ...
    update_set = ", ".join([
        f'"{col}" = EXCLUDED."{col}"' for col in columns
        if col not in ("year", "month", "district_code", "country_code", "hs_code", "trade_type")
    ])
...
async def ingest_trade_summary(
    ...
    try:
        ...
        # Fetch trade summary
        summary = await client.get_trade_summary_by_country(
            year=year,
            month=month
        )
...
async def _insert_trade_summary_data(
    ...
    columns = [
        "year", "month", "country_code", "country_name",
        "exports_value", "imports_value", "total_trade", "trade_balance"
    ]
