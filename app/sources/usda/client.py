...
class USDAClient:
    ...
    SECTORS = ["CROPS", "ANIMALS & PRODUCTS", "ECONOMICS", "ENVIRONMENTAL", "DEMOGRAPHICS"]
    ...
    async def _api_request(
        ...
        async with self.semaphore:
            ...
            for attempt in range(self.max_retries):
                try:
                    logger.debug(f"Fetching USDA {data_id} (attempt {attempt+1}/{self.max_retries})")
                    ...
                    if "data" in data:
                        records = data["data"]
                        logger.debug(f"Successfully fetched {len(records)} {data_id} records")
                        return records
                    ...
                    logger.warning(f"HTTP error fetching USDA data (attempt {attempt+1}): {e}")
                    ...
                    logger.warning(f"Request error fetching USDA data (attempt {attempt+1}): {e}")
            ...
            raise Exception(f"Failed to fetch USDA data after {self.max_retries} attempts")
...
DATA_UNITS = {
    "PRODUCTION": {"CORN": "BU", "SOYBEANS": "BU", "WHEAT": "BU", "COTTON": "480 LB BALES"},
