...
async def prepare_usda_table(conn, data_type: str, commodity: Optional[str] = None) -> str:
    """
...
async def ingest_crop_production(
    ...
    try:
        ...
        logger.info(f"Ingested {rows_inserted} production records for {commodity} {year}")
...
async def ingest_livestock_inventory(
    ...
    try:
        ...
        logger.info(f"Ingested {rows_inserted} livestock records for {commodity} {year}")
        return rows_inserted
...
async def _insert_usda_records(
    ...
    insert_columns = [
        col for col in CROP_PRODUCTION_COLUMNS.keys()
    ...
    for record in records:
        ...
        update_cols = [
            f"{col} = EXCLUDED.{col}"
            for col in insert_columns
            if col not in ("commodity_desc", "year", "state_name", "statisticcat_desc", 
                          "reference_period_desc", "class_desc", "domain_desc")
        ]
