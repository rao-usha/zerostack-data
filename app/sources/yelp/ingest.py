...
async def prepare_table_for_yelp_data(db: Session, dataset: str) -> Dict[str, Any]:
    """
...
async def ingest_businesses_by_location(
    ...
    try:
        ...
        api_response = await client.search_businesses(
            location=location, term=term, categories=categories, limit=min(limit, 50)
        )
...
async def ingest_business_categories(
    db: Session, job_id: int, api_key: str = None
) -> Dict[str, Any]:
    """
...
async def _insert_businesses(
    ...
    update_set = ", ".join(
        [f"{col} = EXCLUDED.{col}" for col in columns if col != "yelp_id"]
    )
...
async def _insert_categories(
    ...
    column_list = ", ".join(columns)
    update_set = ", ".join(
        [f"{col} = EXCLUDED.{col}" for col in columns if col != "alias"]
    )
